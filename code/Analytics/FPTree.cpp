 ...
 * A prefix path is a list of Items that reflects a path from the bottom of
 * the tree to the root (but excluding the root), following along the path of
 * an FPNode that has the ItemID itemID. Because it is a list of Items, it
 * also includes both the ItemID and the SupportCount. The original
 * SupportCount of the Item encapsulated by the FPNode is erased and replaced
...
void FPTree::addTransaction(Transaction transaction) {
    ...
    foreach (Item item, transaction) {
        if (currentNode->hasChild(item.id)) {
            ...
            // item, so reuse it: increase its support count.
        ...
        // We've processed this item in the transaction, time to move on to
        // the next!
